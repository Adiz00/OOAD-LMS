...
const TeacherAnnouncements = ({ userDetail }) => {
    ...
    fetch(`http://${IP}:8000/api/courses`)
      ...
      .then((data) => {
        ...
        const filteredCourses = data.data.filter(
          (course) => course?.course_name === userDetail?.teacher_course && course?.course_Institute?.toLowerCase() === userDetail?.teacher_Institute?.toLowerCase()
        );
        setCourses(filteredCourses);
        console.log("filteredCourses ", filteredCourses);
        const announcements =
          filteredCourses.length > 0
            ? filteredCourses[0].course_Announcements.map((announcement) => ({
  ...
  const handleSave = () => {
    const announcementData = {
      courseName: userDetail.teacher_course,
      courseInstitute: userDetail.teacher_Institute,
    ...
    fetch(`http://${IP}:8000/api/announcement`, {
      ...
      .then((data) => {
        console.log(data);
        console.log("in announcements", announcementData);
